...
class MazeHunter extends Phaser.Scene {
  ...
  private boostersUsed = [false, false, false];
  ...
  private createHud() {
    ...
    this.candyHudLayer = this.add.container(0, 0).setDepth(5);
    this.boosterBar = this.add.container(0, 0).setDepth(7);
  ...
  private layoutHud() {
    ...
    this.scoreLabel.setDepth(5);
    this.scoreText.setDepth(5);
    this.movesLabel.setDepth(5);
    this.movesText.setDepth(5);
    this.objectiveText.setDepth(5);
    this.objectiveBadgeText.setDepth(5);
    this.levelText.setDepth(5);
    this.levelBadgeText.setDepth(5);
    this.starsText.setDepth(5);
    this.timerText.setDepth(5);
    this.livesText.setDepth(5);
  }

  private drawBoosterBar() {
    if (!this.boosterBar) return;

    this.boosterBar.removeAll(true);
    const { width, height, portrait } = this.layout;
    const y = portrait ? height - 48 : height - 42;
    const barW = Math.min(width - 34, 360);
    const barH = 50;
    const x = width / 2 - barW / 2;

    const panel = this.add.graphics();
    panel.fillStyle(0xff78b9, 0.96);
    panel.fillRoundedRect(x, y - barH / 2, barW, barH, 25);
    panel.lineStyle(2, 0xffd8eb, 0.95);
    panel.strokeRoundedRect(x, y - barH / 2, barW, barH, 25);
    this.boosterBar.add(panel);

    const boosters = [
      { icon: "🍭", label: "+5", color: 0xffd9ef },
      { icon: "💖", label: "POWER", color: 0xd9f8ff },
      { icon: "💡", label: "HINT", color: 0xfff1c7 },
    ];

    const spacing = barW / boosters.length;
    boosters.forEach((b, i) => {
      const cx = x + spacing * (i + 0.5);
      const orb = this.add.circle(cx, y, 18, 0xfff7fd, 1).setStrokeStyle(2, 0xffb2d5, 0.9);
      const icon = this.add.text(cx, y - 1, b.icon, { fontFamily: "Arial", fontSize: "18px" }).setOrigin(0.5);
      const label = this.add.text(cx, y + 25, b.label, { fontFamily: "Trebuchet MS, Arial, sans-serif", fontSize: "7px", fontStyle: "bold", color: "#fff" }).setOrigin(0.5);
      this.boosterBar.add([orb, icon, label]);
    });
  ...
  private continueToNextLevel() {
    ...
    this.boostersUsed = [false, false, false];
    this.levelCompletePending = false;
    this.busy = false;
  ...
  private showIntroScreen() {
    ...
    const info = this.add.text(width / 2, py + 163, `♥ ${this.lives}     ⏱ ${this.formatTime(this.getLevelConfig().time)}     ✦ ${this.getLevelConfig().moves} MOVES`, {
      fontFamily: "Trebuchet MS, Arial, sans-serif", fontStyle: "bold",
    ...
    boosters.forEach(([icon, label], i) => {
      const cx = width / 2 + (i - 1) * Math.min(92, width * 0.20);
      const orb = this.add.circle(cx, py + 235, 29, 0xffeef8, 1).setStrokeStyle(3, 0xffb1d5, 0.95).setDepth(303);
      this.add.text(cx, py + 235, icon, { fontFamily: "Arial", fontSize: "24px" }).setOrigin(0.5).setDepth(304);
      this.add.text(cx, py + 274, label, { fontFamily: "Trebuchet MS, Arial, sans-serif", fontStyle: "bold", fontSize: "8px", color: "#9b4f84" }).setOrigin(0.5).setDepth(304);
    });
