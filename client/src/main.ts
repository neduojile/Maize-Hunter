...
import { AIDirector, type AIDirectorDecision, type AIDirectorSnapshot } from "./aiDirector";
...
const CANDIES: Array<{
  ...
  },
  {
    ...
    dark: 0x245ba5,
    glow: 0xa5d4ff,
    asset: "fruit-blueberry",
  },
  {
    color: 0xff4b62,
    dark: 0xb51f39,
    glow: 0xffa2ad,
    asset: "fruit-watermelon",
  },
  {
    color: 0xffd34b,
    dark: 0xa8730d,
    glow: 0xffef91,
    asset: "fruit-pineapple",
  },
];
...
class MazeHunter extends Phaser.Scene {
  ...
  // AI Game Director
  private aiDirector = new AIDirector();
  private aiEvent?: AIDirectorDecision;
  private aiEventEndsAt = 0;
  private aiEventMultiplier = 1;
  private aiTimerPressureRequested = false;
  ...
  preload() {
    ...
    this.load.image("fruit-strawberry", "/assets/fruits/strawberry.svg");
    this.load.image("fruit-orange", "/assets/fruits/orange.svg");
    this.load.image("fruit-grapes", "/assets/fruits/grapes.svg");
    this.load.image("fruit-blueberry", "/assets/fruits/blueberry.svg");
    this.load.image("fruit-watermelon", "/assets/fruits/watermelon.svg");
    this.load.image("fruit-pineapple", "/assets/fruits/pineapple.svg");
  }
  ...
  private createHud() {
    ...
    this.starsText = this.add.text(0, 0, "* * *", {
  ...
  private getObjectiveLabel(): string {
    ...
    if (config.objective === "combo") return `MISSION REACH COMBO x${config.objectiveAmount}`;
  ...
  private createPiece(
    ...
    // STRAWBERRY
    // --------------------------------------------------------
    if (data.asset === "fruit-strawberry") {
      fruit.fillStyle(0xe92f52, 1);
      fruit.fillTriangle(
        0,
        r * 1.05,
        -r * 0.88,
    ...
    // ORANGE
    // --------------------------------------------------------
    else if (data.asset === "fruit-orange") {
      fruit.fillStyle(0xf28a22, 1);
      fruit.fillCircle(0, 0, r * 0.92);
    ...
    // GRAPES
    // --------------------------------------------------------
    else if (data.asset === "fruit-grapes") {
      const grapeRadius = r * 0.28;

      fruit.fillStyle(0x713fc5, 1);
    ...
    // BLUEBERRY
    // --------------------------------------------------------
    else if (data.asset === "fruit-blueberry") {
      fruit.fillStyle(0x3989ed, 1);
      fruit.fillCircle(0, 0, r * 0.88);
      ...
      fruit.strokeCircle(0, 0, r * 0.88);

      // crown
      fruit.fillStyle(0x5d9d3f, 1);
    ...
    // WATERMELON
    // --------------------------------------------------------
    else if (data.asset === "fruit-watermelon") {
      fruit.fillStyle(0xef4059, 1);
      ...
      for (const [sx, sy] of watermelonSeeds) {
        fruit.fillEllipse(
          sx,
          sy,
          Math.max(2, r * 0.08),
    ...
    // PINEAPPLE
    // --------------------------------------------------------
    else if (data.asset === "fruit-pineapple") {
      fruit.fillStyle(0xf3bf2d, 1);

      fruit.fillRoundedRect(
        -r * 0.58,
        -r * 0.50,
        r * 1.16,
        r * 1.42,
  ...
  private addSpecialVisual(
    ...
    // Small special indicator.
    // It stays INSIDE the fruit and never covers neighboring cells.
    const r = cell * 0.32;
  ...
  private resolveMatches(
    ...
    const aiMultiplier = this.getActiveAIMultiplier();
    const finalPoints = Math.round(points * aiMultiplier);
  ...
  private buildAIDirectorSnapshot(
    reason: AIDirectorSnapshot["reason"]
  ): AIDirectorSnapshot {
    return {
      level: this.level,
      score: this.score,
      ...
      recentClears: this.totalCleared,
      reason,
    };
  }

  private async requestAIDirector(
    reason: AIDirectorSnapshot["reason"]
  ) {
    const snapshot = this.buildAIDirectorSnapshot(reason);
    console.log("[MAZE HUNTER AI] Sending snapshot to Antseed...", snapshot);

    const decision = await this.aiDirector.decide(snapshot);

    console.log(
      "[MAZE HUNTER AI] Director decision:",
      decision
    );

    if (!decision) return;

    this.aiEvent = decision;
    this.aiEventEndsAt = Date.now() + decision.duration * 1000;
    this.aiEventMultiplier = decision.multiplier;

    if (decision.event === "bonus_time") {
      const bonusSeconds = Math.max(
        5,
        Math.min(decision.target ?? 8, 12)
      );
    ...
    this.aiTimerPressureRequested = false;
  }

  private getActiveAIMultiplier(): number {
    if (!this.aiEvent) return 1;

    if (Date.now() >= this.aiEventEndsAt) {
      this.aiEvent = undefined;
      this.aiEventMultiplier = 1;
      return 1;
    }

    if (
      this.aiEvent.event === "combo_hunt" &&
      this.combo < (this.aiEvent.target ?? 3)
    ) {
      return 1;
    }

    return this.aiEventMultiplier;
  }
  private ensureAudio() {
    if (this.soundReady) {
  ...
  private updateHud() {
    ...
    this.objectiveBadgeText?.setText(this.getObjectiveLabel().replace(/^MISSION /, "").replace(" CRYSTALS", ""));
    this.levelBadgeText?.setText(String(this.level));
    this.starsText?.setText(this.level >= 3 ? "* * *" : this.level === 2 ? "* * -" : "* - -");
    this.layoutHud();
  }
  ...
  private startLevelTimer() {
    ...
    this.timerEvent = this.time.addEvent({
      ...
      if (
        this.timeLeft <= 30 &&
        !this.aiTimerPressureRequested
      ) {
        this.aiTimerPressureRequested = true;
        void this.requestAIDirector("timer_pressure");
      }
        this.updateHud();
  ...
  private continueToNextLevel() {
    ...
    void this.requestAIDirector("level_start");
    this.resetHintTimer();
  ...
  private showLevelCompleteOverlay(bonus: number) {
    ...
    const moveBonus = this.moves * 15;
    const nextLevel = this.level + 1;

    // Soft game-wide overlay.
    ...
    // Main candy-style card.
    const panelW = Math.min(width - 32, 390);
    const panelH = Math.min(height - 120, 430);
    ...
    panel.lineStyle(3, 0xff7caf, 0.95);
    ...
    const title = this.add.text(
      ...
      {
        fontFamily: "Arial Black, Arial, sans-serif",
        fontSize: `${Math.min(30, width * 0.072)}px`,
        color: "#ff4f91",
        fontStyle: "bold",
        align: "center",
      }
    ...
    const subtitle = this.add.text(
      ...
      {
        fontFamily: "Arial, sans-serif",
        fontSize: `${Math.min(13, width * 0.032)}px`,
        color: "#a85b87",
        fontStyle: "bold",
        letterSpacing: 2,
        align: "center",
    ...
    const timeValue = this.add.text(
      ...
      {
        fontFamily: "Arial Black, Arial, sans-serif",
        fontSize: `${Math.min(15, width * 0.037)}px`,
        color: "#ff4f91",
        fontStyle: "bold",
      }
    )
    ...
    const moveValue = this.add.text(
      panelX + panelW / 2 - 30,
      rowStart + 42,
      `+${moveBonus.toLocaleString()}`,
      {
        fontFamily: "Arial Black, Arial, sans-serif",
        fontSize: `${Math.min(15, width * 0.037)}px`,
        color: "#ff4f91",
    ...
    const totalLabel = this.add.text(
      ...
      {
        fontFamily: "Arial Black, Arial, sans-serif",
        fontSize: `${Math.min(16, width * 0.039)}px`,
        color: "#5f315f",
        fontStyle: "bold",
      }
    )
    ...
    const totalValue = this.add.text(
      ...
      {
        fontFamily: "Arial Black, Arial, sans-serif",
        fontSize: `${Math.min(20, width * 0.048)}px`,
        color: "#ff4f91",
        fontStyle: "bold",
      }
    )
    ...
    const buttonW = Math.min(panelW - 58, 300);
    const buttonH = 58;
    const buttonY = panelY + panelH / 2 - 58;
    ...
    const buttonText = this.add.text(
      ...
      {
        fontFamily: "Arial Black, Arial, sans-serif",
        fontSize: `${Math.min(17, width * 0.042)}px`,
        color: "#ffffff",
        fontStyle: "bold",
        letterSpacing: 1,
      }
    ...
    // Decorative dots.
    const dots = [
      { x: panelX - panelW / 2 + 28, y: panelY - panelH / 2 + 25 },
      { x: panelX + panelW / 2 - 28, y: panelY - panelH / 2 + 25 },
      { x: panelX - panelW / 2 + 30, y: buttonY },
      { x: panelX + panelW / 2 - 30, y: buttonY },
    ];

    const decorations = dots.map((dot) =>
      this.add.circle(dot.x, dot.y, 5, 0xffb7d2, 0.9)
        .setDepth(222)
    );
    ...
    this.tweens.add({
      targets: [button, buttonText, buttonHighlight],
      scaleX: 1.025,
      scaleY: 1.025,
      duration: 720,
      repeat: -1,
    ...
    button.on("pointerup", () => {
      ...
      this.playTone(783.99, 0.12, 0.05, "triangle");
  ...
  private showIntroScreen() {
    ...
    const objective = this.add.text(width / 2, py + 113, this.getObjectiveLabel().replace("MISSION ", ""), {
      fontFamily: "Trebuchet MS, Arial, sans-serif", fontStyle: "bold",
      fontSize: `${Math.min(21, width * 0.047)}px`, color: "#873b74",
      align: "center", wordWrap: { width: panelW - 54 },
    }).setOrigin(0.5).setDepth(303);

    const info = this.add.text(width / 2, py + 163, `LIVES ${this.lives}     ${this.formatTime(this.getLevelConfig().time)}     ${this.getLevelConfig().moves} MOVES`, {
      fontFamily: "Trebuchet MS, Arial, sans-serif", fontStyle: "bold",
  ...
  private showEndScreen(won: boolean) {
    ...
    const detail = this.add.text(
      width / 2,
      panelY + 192,
      `LEVEL ${this.level.toString().padStart(2, "0")}   |   BEST COMBO x${this.bestCombo}`,
      {
        fontFamily: "Trebuchet MS, Arial, sans-serif",
