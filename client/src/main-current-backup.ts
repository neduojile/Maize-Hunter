...
class MazeHunter extends Phaser.Scene {
  ...
  private drawBoosterBar() {
    ...
    const boosters = [
      { icon: "ðŸ­", label: "+5", color: 0xffd9ef },
      { icon: "ðŸ’–", label: "POWER", color: 0xd9f8ff },
      { icon: "ðŸ’¡", label: "HINT", color: 0xfff1c7 },
    ];
  ...
  private maybeSpawnBonusBomb() {
    ...
    const label = this.add.text(x, y - cell * 0.55, "ðŸ’£", {
  ...
  private showIntroScreen() {
    ...
    const boosters = [
      ["ðŸ’£", "POWER"], ["ðŸŒˆ", "SPECIAL"], ["ðŸ­", "BOOST"],
    ];
    boosters.forEach(([icon, label], i) => {
      const cx = width / 2 + (i - 1) * Math.min(92, width * 0.20);
      this.add.circle(cx, py + 235, 29, 0xffeef8, 1).setStrokeStyle(3, 0xffb1d5, 0.95).setDepth(303);
