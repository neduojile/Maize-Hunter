...
class MazeHunter extends Phaser.Scene {
  ...
  private layoutHud() {
    const { width, height, portrait, hudTop, titleSize, subtitleSize, statSize } = this.layout;

    this.hudFrame.clear();
    this.progressBar.clear();

    const headerH = portrait ? 136 : 98;
    const x = 12;
    const w = width - 24;

    // Light candy-world header: compact, playful, and intentionally transparent.
    this.hudFrame.fillStyle(0xfffbff, 0.88);
    this.hudFrame.fillRoundedRect(x, 10, w, headerH, 25);
    this.hudFrame.lineStyle(3, 0xffffff, 0.95);
    this.hudFrame.strokeRoundedRect(x, 10, w, headerH, 25);
    this.hudFrame.lineStyle(2, 0xff8fc5, 0.72);
    this.hudFrame.strokeRoundedRect(x + 4, 14, w - 8, headerH - 8, 21);

    // Small candy dots make the panel feel connected to the world art.
    const dots = [
      [x + 14, 25, 0x63d7ef], [x + w - 14, 25, 0xffcf57],
      [x + 14, headerH - 1, 0xff75ad], [x + w - 14, headerH - 1, 0xa46cf1],
    ];
    for (const [dx, dy, color] of dots) {
      this.hudFrame.fillStyle(color, 0.95);
      this.hudFrame.fillCircle(dx, dy, 3.5);
    }

    this.titleText.setPosition(width / 2, hudTop + 18).setFontSize(Math.min(titleSize * 0.82, 36));
    this.subtitleText.setPosition(width / 2, hudTop + 43).setFontSize(Math.max(10, subtitleSize * 0.82));

    if (portrait) {
      const pillY = hudTop + 53;
      const pillW = Math.min(width * 0.37, 145);
      const pillH = 42;
      const gap = 8;
      const leftX = width / 2 - pillW - gap / 2;
      const rightX = width / 2 + gap / 2;

      this.drawStatCard(leftX, pillY, pillW, pillH, 0xff72b8);
      this.drawStatCard(rightX, pillY, pillW, pillH, 0xffc84f);

      this.scoreLabel.setPosition(leftX + 11, pillY + 11).setOrigin(0, 0.5).setFontSize(9);
      this.scoreText.setPosition(leftX + 11, pillY + 28).setOrigin(0, 0.5).setFontSize(Math.max(19, statSize * 0.72));

      this.movesLabel.setPosition(rightX + 11, pillY + 11).setOrigin(0, 0.5).setFontSize(9);
      this.movesText.setPosition(rightX + pillW - 11, pillY + 27).setOrigin(1, 0.5).setFontSize(Math.max(20, statSize * 0.76));

      this.timerText.setPosition(48, 29).setFontSize(15);
      this.livesText.setPosition(width - 48, 29).setFontSize(13);

      const missionY = pillY + pillH + 19;
      this.levelText.setText(`LEVEL ${this.level.toString().padStart(2, "0")} • ${this.getLevelConfig().name}`).setPosition(width / 2, missionY).setFontSize(9);
      this.objectiveText.setText(this.getObjectiveLabel().replace("MISSION • ", "")).setPosition(width / 2, missionY + 14).setFontSize(10);

      const barW = Math.min(width - 70, 300);
      this.drawProgressBar(width / 2 - barW / 2, missionY + 24, barW, 8);
      this.comboText.setPosition(width / 2, height * 0.89);
      this.comboBuddy?.setPosition(width / 2 - 72, height * 0.89);
    } else {
      const pillY = hudTop + 4;
      const pillW = Math.min(width * 0.16, 150);
      const pillH = 46;
      this.drawStatCard(18, pillY, pillW, pillH, 0xff72b8);
      this.drawStatCard(28 + pillW, pillY, pillW, pillH, 0xffc84f);

      this.scoreLabel.setPosition(30, pillY + 11).setOrigin(0, 0.5).setFontSize(9);
      this.scoreText.setPosition(30, pillY + 30).setOrigin(0, 0.5).setFontSize(statSize * 0.78);
      this.movesLabel.setPosition(40 + pillW, pillY + 11).setOrigin(0, 0.5).setFontSize(9);
      this.movesText.setPosition(18 + 2 * pillW, pillY + 29).setOrigin(1, 0.5).setFontSize(statSize * 0.8);

      const center = width / 2;
      this.titleText.setPosition(center, hudTop + 18).setFontSize(Math.min(titleSize * 0.72, 32));
      this.subtitleText.setPosition(center, hudTop + 40).setFontSize(10);
      this.levelText.setPosition(center, pillY + 12).setFontSize(9);
      this.objectiveText.setPosition(center, pillY + 27).setFontSize(10);
      this.drawProgressBar(center - 120, pillY + 37, 240, 8);
      this.timerText.setPosition(width - 72, pillY + 16).setFontSize(15);
      this.livesText.setPosition(width - 72, pillY + 39).setFontSize(11);
      this.comboText.setPosition(width * 0.82, height * 0.82);
      this.comboBuddy?.setPosition(width * 0.82 - 65, height * 0.82);
    }
    ...
    this.subtitleText.setDepth(4);
    this.scoreLabel.setDepth(4);
    this.scoreText.setDepth(4);
    this.movesLabel.setDepth(4);
    this.movesText.setDepth(4);
    this.objectiveText.setDepth(4);
  ...
  private drawStatCard(x: number, y: number, w: number, h: number, accent: number) {
    this.hudFrame.fillStyle(0xffffff, 0.78);
    this.hudFrame.fillRoundedRect(x, y, w, h, 14);
    this.hudFrame.lineStyle(2, accent, 0.58);
    this.hudFrame.strokeRoundedRect(x, y, w, h, 14);
    this.hudFrame.fillStyle(accent, 0.10);
    this.hudFrame.fillRoundedRect(x + 2, y + 2, w - 4, h * 0.48, 12);
    this.hudFrame.fillStyle(0xffffff, 0.75);
    this.hudFrame.fillCircle(x + w - 11, y + 11, 2.5);
  }
  ...
  private addCandyIdleAnimation(container: Phaser.GameObjects.Container) {
    // Never pulse the candy size. Keep the character stable and animate it with a gentle float + tilt.
    const baseY = container.y;
    this.tweens.add({
      targets: container,
      y: baseY - Phaser.Math.FloatBetween(1.5, 3.5),
      angle: Phaser.Math.FloatBetween(-2.2, 2.2),
      duration: 1250 + Phaser.Math.Between(0, 450),
      yoyo: true,
      repeat: -1,
      ease: "Sine.InOut",
  ...
  private maybeSpawnBonusBomb() {
    ...
    const label = this.add.text(x, y - cell * 0.55, "💣", {
