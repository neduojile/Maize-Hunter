...
class MazeHunter extends Phaser.Scene {
  ...
  private drawBackground() {
    ...
    // Deep cinematic base.
    this.background.fillStyle(0x030914, 1);
    this.background.fillRect(0, 0, width, height);

    // Large atmospheric light fields.
    this.background.fillStyle(0x073047, 0.62);
    this.background.fillCircle(width * 0.08, height * 0.18, Math.min(width, height) * 0.42);

    this.background.fillStyle(0x182052, 0.58);
    this.background.fillCircle(width * 0.94, height * 0.68, Math.min(width, height) * 0.48);

    this.background.fillStyle(0x06433f, 0.24);
    this.background.fillCircle(width * 0.52, height * 0.58, Math.min(width, height) * 0.54);

    // Subtle technical grid.
    const grid = Math.max(34, this.layout.cell * 0.9);
    this.background.lineStyle(1, 0x2b6273, 0.11);
    ...
    // Floating crystal particles / constellation points.
    const seed = [
      [0.06, 0.31, 3], [0.16, 0.47, 2], [0.27, 0.14, 2],
      [0.79, 0.13, 3], [0.91, 0.25, 2], [0.73, 0.43, 2],
      [0.11, 0.78, 2], [0.25, 0.88, 3], [0.82, 0.86, 2],
      [0.94, 0.54, 3], [0.58, 0.12, 2], [0.48, 0.88, 2],
    ];

    for (const [px, py, size] of seed) {
      this.background.fillStyle(0x8cf9ef, 0.22);
      this.background.fillCircle(width * px, height * py, size);
      this.background.fillStyle(0xffffff, 0.14);
      this.background.fillCircle(width * px - size * 0.35, height * py - size * 0.35, Math.max(1, size * 0.45));
    }

    // Vignette-like edge bands.
    this.background.fillStyle(0x000000, 0.18);
    this.background.fillRect(0, 0, width, 8);
    this.background.fillRect(0, height - 8, width, 8);
    this.background.fillRect(0, 0, 8, height);
    this.background.fillRect(width - 8, 0, 8, height);
  }
  ...
  private createHud() {
    ...
    this.powerBar = this.add.graphics().setDepth(3);
  }
  ...
  private layoutHud() {
    ...
    // Premium HUD header.
    this.hudFrame.fillStyle(0x061521, 0.72);
    this.hudFrame.fillRoundedRect(12, 10, width - 24, portrait ? 142 : 104, 22);
    this.hudFrame.lineStyle(1, 0x2f8390, 0.55);
    this.hudFrame.strokeRoundedRect(12, 10, width - 24, portrait ? 142 : 104, 22);
    this.hudFrame.lineStyle(1, 0x73eee2, 0.13);
    this.hudFrame.strokeRoundedRect(17, 15, width - 34, portrait ? 132 : 94, 18);

    this.titleText
      .setPosition(width / 2, hudTop + titleSize * 0.48)
      .setFontSize(titleSize)
      .setShadow(0, 5, '#000000', 10, true, true);
    ...
    if (portrait) {
      const cardY = hudTop + titleSize + 42;
      const cardW = Math.min(width * 0.38, 155);
      const cardH = 52;
      const gap = 10;
      const leftX = width / 2 - cardW - gap / 2;
      const rightX = width / 2 + gap / 2;

      this.drawStatCard(leftX, cardY, cardW, cardH, 0x3fa8ff);
      this.drawStatCard(rightX, cardY, cardW, cardH, 0xffc936);

      this.scoreLabel.setPosition(leftX + 14, cardY + 12).setOrigin(0, 0.5).setFontSize(smallSize);
      this.scoreText.setPosition(leftX + 14, cardY + 34).setOrigin(0, 0.5).setFontSize(statSize * 0.86);

      this.movesLabel.setPosition(rightX + 14, cardY + 12).setOrigin(0, 0.5).setFontSize(smallSize);
      this.movesText.setPosition(rightX + cardW - 14, cardY + 33).setOrigin(1, 0.5).setFontSize(statSize);

      const progressY = cardY + cardH + 24;
      this.objectiveText
        .setText(this.getObjectiveLabel())
        .setPosition(width / 2, progressY - 7)
        .setFontSize(Math.max(11, smallSize));

      const barW = Math.min(width - 48, 330);
      const barX = width / 2 - barW / 2;
      this.drawProgressBar(barX, progressY + 13, barW, 9);

      this.comboText.setPosition(width / 2, height * 0.93);
    } else {
      const cardY = hudTop + 4;
      const cardW = Math.min(width * 0.19, 155);
      const cardH = 58;

      this.drawStatCard(width * 0.06, cardY, cardW, cardH, 0x3fa8ff);
      this.drawStatCard(width * 0.25, cardY, cardW, cardH, 0xffc936);

      this.scoreLabel.setPosition(width * 0.06 + 14, cardY + 14).setOrigin(0, 0.5).setFontSize(smallSize);
      this.scoreText.setPosition(width * 0.06 + 14, cardY + 39).setOrigin(0, 0.5).setFontSize(statSize * 0.88);
      this.movesLabel.setPosition(width * 0.25 + 14, cardY + 14).setOrigin(0, 0.5).setFontSize(smallSize);
      this.movesText.setPosition(width * 0.25 + cardW - 14, cardY + 37).setOrigin(1, 0.5).setFontSize(statSize);

      const barW = Math.min(width * 0.34, 340);
      const barX = width * 0.48;
      this.objectiveText.setText(this.getObjectiveLabel()).setPosition(barX + barW / 2, cardY + 13).setFontSize(smallSize);
      this.drawProgressBar(barX, cardY + 36, barW, 9);
      this.comboText.setPosition(width * 0.78, height * 0.86);
    }

    const config = this.getLevelConfig();
    this.levelText
      .setText(`LEVEL ${this.level} • ${config.name}`)
      .setPosition(width / 2, portrait ? hudTop + titleSize + 24 : hudTop + 76)
      .setFontSize(Math.max(10, smallSize * 0.9));

    this.timerText
      .setPosition(portrait ? width * 0.18 : width * 0.88, portrait ? hudTop + 28 : hudTop + 22)
      .setFontSize(Math.max(15, statSize * 0.52))
      .setColor(this.timeLeft <= 30 ? "#ff6685" : "#ffd85a");

    this.livesText
      .setText("♥ ".repeat(this.lives).trim() || "NO LIVES")
      .setPosition(portrait ? width * 0.82 : width * 0.88, portrait ? hudTop + 28 : hudTop + 48)
      .setFontSize(Math.max(11, smallSize));

    this.drawPowerBar();

    this.hudFrame.setDepth(1);
  ...
  private drawStatCard(x: number, y: number, w: number, h: number, accent: number) {
    this.hudFrame.fillStyle(0x091b28, 0.94);
    this.hudFrame.fillRoundedRect(x, y, w, h, 14);
    this.hudFrame.lineStyle(1, accent, 0.38);
    this.hudFrame.strokeRoundedRect(x, y, w, h, 14);
    this.hudFrame.fillStyle(accent, 0.08);
    this.hudFrame.fillRoundedRect(x + 2, y + 2, w - 4, h * 0.42, 12);
    this.hudFrame.fillStyle(accent, 0.7);
    this.hudFrame.fillRoundedRect(x + 9, y + 9, 3, h - 18, 2);
  }

  private drawProgressBar(x: number, y: number, w: number, h: number) {
    const progress = this.getObjectiveProgress();
    this.progressBar.fillStyle(0x020b12, 0.95);
    this.progressBar.fillRoundedRect(x, y, w, h, h / 2);
    this.progressBar.lineStyle(1, 0x3a7580, 0.65);
    this.progressBar.strokeRoundedRect(x, y, w, h, h / 2);
    this.progressBar.fillStyle(progress >= 1 ? 0xffd24d : 0x58e5c5, 0.95);
    this.progressBar.fillRoundedRect(x + 2, y + 2, Math.max(4, (w - 4) * progress), h - 4, (h - 4) / 2);
  }
  ...
  private getObjectiveLabel(): string {
    const config = this.getLevelConfig();
    if (config.objective === "score") return `MISSION • REACH ${config.objectiveAmount.toLocaleString()}`;
    if (config.objective === "crystals") return `MISSION • CLEAR ${config.objectiveAmount} CRYSTALS`;
    if (config.objective === "combo") return `MISSION • REACH COMBO ×${config.objectiveAmount}`;
    return `MISSION • CREATE ${config.objectiveAmount} SPECIALS`;
  }
  ...
  private drawPowerBar() {
    ...
    const w = Math.min(width * (portrait ? 0.42 : 0.28), 240);
    const x = width / 2 - w / 2;
    const y = portrait ? height * 0.79 : height * 0.90;
    this.powerBar.clear();
    this.powerBar.fillStyle(0x020b12, 0.9);
    this.powerBar.fillRoundedRect(x, y, w, 7, 4);
    this.powerBar.lineStyle(1, 0x5bd0d1, 0.3);
    this.powerBar.strokeRoundedRect(x, y, w, 7, 4);
    this.powerBar.fillStyle(this.power >= 100 ? 0xffd34d : 0x7a8cff, 0.95);
    this.powerBar.fillRoundedRect(x + 1, y + 1, Math.max(3, (w - 2) * Phaser.Math.Clamp(this.power / 100, 0, 1)), 5, 3);
  ...
  private layoutBoard() {
    ...
    const outer = Math.max(14, cell * 0.18);
    const radius = Math.max(18, cell * 0.30);

    // Deep floating shadow.
    this.boardFrame.fillStyle(0x000000, 0.48);
    this.boardFrame.fillRoundedRect(
      boardX - outer + 8, boardY - outer + 12,
      boardSize + outer * 2, boardSize + outer * 2, radius + 4
    );

    // Outer crystal frame.
    this.boardFrame.fillStyle(0x06131f, 0.98);
    this.boardFrame.fillRoundedRect(
      boardX - outer, boardY - outer,
      boardSize + outer * 2, boardSize + outer * 2, radius
    );
    this.boardFrame.lineStyle(3, 0x1f9eb0, 0.75);
    this.boardFrame.strokeRoundedRect(
      boardX - outer, boardY - outer,
      boardSize + outer * 2, boardSize + outer * 2, radius
    );
    this.boardFrame.lineStyle(1, 0x8fffee, 0.26);
    this.boardFrame.strokeRoundedRect(
      boardX - outer + 6, boardY - outer + 6,
      boardSize + outer * 2 - 12, boardSize + outer * 2 - 12, radius - 5
    );

    // Inner playing surface.
    this.boardFrame.fillStyle(0x071c28, 0.98);
    this.boardFrame.fillRoundedRect(
      boardX, boardY, boardSize, boardSize, Math.max(10, cell * 0.13)
    );

    // Cell separators with a subtle inset look.
    this.boardFrame.lineStyle(1, 0x5bd0d1, 0.10);
    ...
    // Four luminous corner brackets.
    this.boardFrame.lineStyle(2, 0x7affdf, 0.72);
    const b = Math.max(12, cell * 0.22);
  ...
  private drawCandy(
    ...
    shadow.fillEllipse(r * 0.10, r * 0.22, r * 1.55, r * 0.72);

    const aura = this.add.graphics();
    aura.fillStyle(data.glow, 0.12);
    aura.fillCircle(0, 0, r * 1.30);
    ...
    crystal.fillPoints(pts.map((p) => new Phaser.Math.Vector2(p.x, p.y)), true);
    ...
    crystal.fillPoints(inner.map((p) => new Phaser.Math.Vector2(p.x, p.y)), true);

    // Facets.
    crystal.fillStyle(data.glow, 0.34);
    crystal.fillPoints([
      { x: -r * 0.62, y: -r * 0.45 },
      { x: 0, y: -r * 0.82 },
      { x: r * 0.12, y: -r * 0.18 },
    ...
    crystal.fillStyle(0xffffff, 0.38);
    crystal.fillEllipse(-r * 0.28, -r * 0.39, r * 0.42, r * 0.18);
    crystal.fillStyle(0x000000, 0.18);
    crystal.fillPoints([
      { x: r * 0.12, y: -r * 0.18 },
      { x: r * 0.78, y: r * 0.10 },
      { x: r * 0.42, y: r * 0.68 },
    ...
    crystal.lineStyle(Math.max(1.5, cell * 0.022), 0xffffff, 0.25);
    ...
    const icon = this.add
      .text(0, r * 0.03, data.symbol, {
        fontFamily: "Arial Black, Arial, sans-serif",
        fontSize: `${Math.max(12, cell * 0.25)}px`,
        color: "#ffffff",
        stroke: "#ffffff",
        strokeThickness: Math.max(1, cell * 0.014),
      })
      .setOrigin(0.5)
      .setAlpha(0.92);

    container.add([shadow, aura, crystal, icon]);
  ...
  private showLevelCompleteOverlay(bonus: number) {
    ...
    const buttonText = this.add.text(width / 2, height / 2 + 94, `NEXT LEVEL • ${this.level + 1}`, {
      fontFamily: "Arial Black, Arial, sans-serif",
  ...
  private showIntroScreen() {
    ...
    const overlay = this.add.rectangle(0, 0, width, height, 0x02070b, 0.86)
      .setOrigin(0)
      .setDepth(300);

    const panelW = Math.min(width - 30, 440);
    const panelH = portrait ? Math.min(height - 34, 570) : Math.min(height - 30, 420);
    const panelX = width / 2 - panelW / 2;
    const panelY = height / 2 - panelH / 2;

    const panel = this.add.graphics().setDepth(301);
    panel.fillStyle(0x081a27, 0.98);
    panel.fillRoundedRect(panelX, panelY, panelW, panelH, 30);
    panel.lineStyle(2, 0x48e0d0, 0.75);
    panel.strokeRoundedRect(panelX, panelY, panelW, panelH, 30);
    panel.lineStyle(1, 0x9bffff, 0.16);
    panel.strokeRoundedRect(panelX + 7, panelY + 7, panelW - 14, panelH - 14, 24);

    const title = this.add.text(width / 2, panelY + 48, "MAZE HUNTER", {
      fontFamily: "Arial Black, Arial, sans-serif",
      fontSize: `${Math.min(38, width * 0.08)}px`,
      color: "#f7fbff",
      stroke: "#06121b",
      strokeThickness: 6,
    }).setOrigin(0.5).setDepth(302);

    const subtitle = this.add.text(width / 2, panelY + 86, "CRYSTAL MAZE", {
      fontFamily: "Arial, sans-serif",
      fontSize: `${Math.min(14, width * 0.032)}px`,
      color: "#6ee7d8",
      letterSpacing: 4,
    }).setOrigin(0.5).setDepth(302);

    const steps = [
      ["01", "SWIPE OR TAP", "Move adjacent crystals to line up 3 or more."],
      ["02", "BUILD SPECIALS", "Make 4, 5, or T/L matches for powerful crystals."],
      ["03", "CLEAR THE MAZE", "Hit the mission before time and moves run out."],
    ];

    const stepObjects: Phaser.GameObjects.GameObject[] = [];
    const startY = panelY + (portrait ? 135 : 130);
    const stepGap = portrait ? 104 : 78;

    steps.forEach(([num, heading, body], index) => {
      const y = startY + index * stepGap;

      const circle = this.add.circle(panelX + 38, y, 17, 0x18a6a0, 1)
        .setStrokeStyle(1, 0x8fffee, 0.65)
        .setDepth(302);

      const n = this.add.text(panelX + 38, y, num, {
        fontFamily: "Arial Black, Arial, sans-serif",
        fontSize: "11px",
        color: "#ffffff",
      }).setOrigin(0.5).setDepth(303);

      const h = this.add.text(panelX + 67, y - 12, heading, {
        fontFamily: "Arial Black, Arial, sans-serif",
        fontSize: `${Math.min(14, width * 0.033)}px`,
        color: "#ffffff",
      }).setOrigin(0, 0.5).setDepth(302);

      const b = this.add.text(panelX + 67, y + 17, body, {
        fontFamily: "Arial, sans-serif",
        fontSize: `${Math.min(12, width * 0.028)}px`,
        color: "#8fa9b5",
        wordWrap: { width: panelW - 92 },
      }).setOrigin(0, 0.5).setDepth(302);

      stepObjects.push(circle, n, h, b);
    });

    const buttonY = panelY + panelH - 52;
    const button = this.add.rectangle(width / 2, buttonY, Math.min(panelW - 48, 300), 54, 0x18a6a0, 1)
      .setInteractive({ useHandCursor: true })
      .setDepth(302);
    button.setStrokeStyle(2, 0x8fffee, 0.72);

    const buttonText = this.add.text(width / 2, buttonY, "ENTER CRYSTAL MAZE", {
      fontFamily: "Arial Black, Arial, sans-serif",
      fontSize: `${Math.min(16, width * 0.038)}px`,
      color: "#ffffff",
    }).setOrigin(0.5).setDepth(303);

    const introObjects: Phaser.GameObjects.GameObject[] = [overlay, panel, title, subtitle, ...stepObjects, button, buttonText];
    introObjects.forEach((obj) => (obj as any).setAlpha(0));
    ...
    button.on("pointerup", () => {
      ...
      title.destroy();
      subtitle.destroy();
      stepObjects.forEach((obj) => obj.destroy());
      button.destroy();
      buttonText.destroy();
      ...
      const pulse = this.add.text(width / 2, height * 0.34, "HUNT STARTED", {
        fontFamily: "Arial Black, Arial, sans-serif",
        fontSize: `${Math.min(25, width * 0.055)}px`,
        color: "#7affdf",
        stroke: "#07121c",
        strokeThickness: 5,
