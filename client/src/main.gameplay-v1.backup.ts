...
class MazeHunter extends Phaser.Scene {
  ...
  private resolveMatches() {
    ...
    for (const match of matches) {
      const piece = this.pieces[match.row][match.col];

      if (!piece) continue;

      this.createBurst(match.row, match.col);
      this.createFloatingScore(match.row, match.col, Math.floor(points / matches.length));
      ...
      this.pieces[match.row][match.col] = null;
    }
  ...
  private shakeBoard() {
    const strength = this.layout.portrait ? 4 : 6;

    this.cameras.main.shake(130, strength / 1000);
  }
