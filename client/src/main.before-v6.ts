...
class MazeHunter extends Phaser.Scene {
  ...
  private createHud() {
    ...
    this.titleText = this.add.text(0, 0, "MAZE HUNTER", titleStyle).setOrigin(0.5);

    this.subtitleText = this.add.text(0, 0, "CRYSTAL MAZE // SECTOR 01", {
      fontFamily: "Trebuchet MS, Arial, sans-serif",
      fontSize: "14px",
      fontStyle: "bold",
      color: "#61e8d7",
    ...
    this.objectiveText = this.add.text(0, 0, "MISSION // REACH 5,000", {
      fontFamily: "Trebuchet MS, Arial, sans-serif",
      fontSize: "13px",
      fontStyle: "bold",
      color: "#ffe06b",
    ...
    this.levelText = this.add.text(0, 0, "LEVEL 01 // AWAKENING", {
      fontFamily: "Trebuchet MS, Arial, sans-serif",
      fontSize: "11px",
      fontStyle: "bold",
      color: "#7cebdc",
    ...
    this.livesText = this.add.text(0, 0, "♥ ♥ ♥", {
      fontFamily: "Trebuchet MS, Arial, sans-serif",
      fontSize: "15px",
      fontStyle: "bold",
      color: "#ff7392",
  ...
  private layoutHud() {
    ...
    const headerH = portrait ? 168 : 112;

    // Top glass command deck.
    this.hudFrame.fillStyle(0x06111c, 0.94);
    this.hudFrame.fillRoundedRect(12, 10, width - 24, headerH, 24);

    this.hudFrame.lineStyle(1.5, 0x4ee8d8, 0.34);
    this.hudFrame.strokeRoundedRect(12, 10, width - 24, headerH, 24);

    this.hudFrame.lineStyle(1, 0xffffff, 0.06);
    this.hudFrame.strokeRoundedRect(18, 16, width - 36, headerH - 12, 19);

    // Header accent rail.
    this.hudFrame.fillStyle(0x5ce7d6, 0.72);
    this.hudFrame.fillRoundedRect(width / 2 - Math.min(90, width * 0.12), 18, Math.min(180, width * 0.24), 3, 2);
    ...
    this.subtitleText
      .setPosition(width / 2, hudTop + titleSize + 8)
      .setFontSize(subtitleSize);

    if (portrait) {
      const cardY = hudTop + titleSize + 43;
      const cardW = Math.min(width * 0.41, 170);
      const cardH = 55;
      const gap = 9;
      const leftX = width / 2 - cardW - gap / 2;
      const rightX = width / 2 + gap / 2;

      this.drawStatCard(leftX, cardY, cardW, cardH, 0x5ba8ff);
      this.drawStatCard(rightX, cardY, cardW, cardH, 0xffd45a);

      this.scoreLabel.setPosition(leftX + 13, cardY + 13).setOrigin(0, 0.5).setFontSize(smallSize);
      this.scoreText.setPosition(leftX + 13, cardY + 37).setOrigin(0, 0.5).setFontSize(statSize * 0.86);

      this.movesLabel.setPosition(rightX + 13, cardY + 13).setOrigin(0, 0.5).setFontSize(smallSize);
      this.movesText.setPosition(rightX + cardW - 13, cardY + 36).setOrigin(1, 0.5).setFontSize(statSize);

      const missionY = cardY + cardH + 25;
      this.levelText
        .setText(`LEVEL ${this.level.toString().padStart(2, "0")} // ${this.getLevelConfig().name}`)
        .setPosition(width / 2, missionY - 14)
        .setFontSize(Math.max(9, smallSize * 0.86));

      this.objectiveText
        .setText(this.getObjectiveLabel().replace("MISSION • ", "MISSION // "))
        .setPosition(width / 2, missionY + 2)
        .setFontSize(Math.max(10, smallSize));

      const barW = Math.min(width - 50, 340);
      this.drawProgressBar(width / 2 - barW / 2, missionY + 16, barW, 10);

      this.timerText
        .setPosition(width * 0.17, hudTop + 27)
        .setFontSize(Math.max(14, statSize * 0.50));

      this.livesText
        .setPosition(width * 0.83, hudTop + 27)
        .setFontSize(Math.max(11, smallSize));

      this.comboText.setPosition(width / 2, height * 0.92);
    } else {
      const cardY = hudTop + 8;
      const cardW = Math.min(width * 0.18, 165);
      const cardH = 60;

      this.drawStatCard(width * 0.045, cardY, cardW, cardH, 0x5ba8ff);
      this.drawStatCard(width * 0.235, cardY, cardW, cardH, 0xffd45a);

      this.scoreLabel.setPosition(width * 0.045 + 14, cardY + 14).setOrigin(0, 0.5).setFontSize(smallSize);
      this.scoreText.setPosition(width * 0.045 + 14, cardY + 40).setOrigin(0, 0.5).setFontSize(statSize * 0.88);

      this.movesLabel.setPosition(width * 0.235 + 14, cardY + 14).setOrigin(0, 0.5).setFontSize(smallSize);
      this.movesText.setPosition(width * 0.235 + cardW - 14, cardY + 38).setOrigin(1, 0.5).setFontSize(statSize);

      const missionX = width * 0.47;
      const barW = Math.min(width * 0.31, 330);

      this.levelText
        .setText(`LEVEL ${this.level.toString().padStart(2, "0")} // ${this.getLevelConfig().name}`)
        .setPosition(missionX + barW / 2, cardY + 13)
        .setFontSize(Math.max(9, smallSize * 0.88));

      this.objectiveText
        .setText(this.getObjectiveLabel().replace("MISSION • ", "MISSION // "))
        .setPosition(missionX + barW / 2, cardY + 28)
        .setFontSize(Math.max(10, smallSize));

      this.drawProgressBar(missionX, cardY + 43, barW, 9);

      this.timerText
        .setPosition(width * 0.83, cardY + 23)
        .setFontSize(Math.max(15, statSize * 0.54));

      this.livesText
        .setPosition(width * 0.92, cardY + 48)
  ...
  private drawStatCard(x: number, y: number, w: number, h: number, accent: number) {
    this.hudFrame.fillStyle(0x081723, 0.98);
    this.hudFrame.fillRoundedRect(x, y, w, h, 15);

    this.hudFrame.lineStyle(1, accent, 0.48);
    this.hudFrame.strokeRoundedRect(x, y, w, h, 15);

    this.hudFrame.fillStyle(accent, 0.06);
    this.hudFrame.fillRoundedRect(x + 2, y + 2, w - 4, h * 0.50, 13);

    this.hudFrame.fillStyle(accent, 0.9);
    this.hudFrame.fillRoundedRect(x + 9, y + 10, 3, h - 20, 2);

    this.hudFrame.fillStyle(0xffffff, 0.07);
    this.hudFrame.fillRoundedRect(x + 17, y + 8, w - 27, 2, 1);
  }

  private drawProgressBar(x: number, y: number, w: number, h: number) {
    const progress = this.getObjectiveProgress();

    this.progressBar.fillStyle(0x01060a, 0.96);
    this.progressBar.fillRoundedRect(x, y, w, h, h / 2);

    this.progressBar.lineStyle(1, 0x5ae3d4, 0.28);
    this.progressBar.strokeRoundedRect(x, y, w, h, h / 2);

    const fill = progress >= 1 ? 0xffd45a : 0x59e7d5;
  ...
  private drawPowerBar() {
    ...
    const x = width / 2 - w / 2;
    const y = portrait ? height * 0.805 : height * 0.905;

    this.powerBar.clear();

    this.powerBar.fillStyle(0x02070c, 0.96);
    this.powerBar.fillRoundedRect(x, y, w, 9, 5);

    this.powerBar.lineStyle(1, 0x7f8dff, 0.42);
    this.powerBar.strokeRoundedRect(x, y, w, 9, 5);

    const progress = Phaser.Math.Clamp(this.power / 100, 0, 1);
    const fill = this.power >= 100 ? 0xffd45a : 0x8b7cff;
  ...
  private layoutBoard() {
    ...
    const outer = Math.max(17, cell * 0.22);
    const radius = Math.max(22, cell * 0.34);

    // Floating shadow.
    this.boardFrame.fillStyle(0x000000, 0.58);
    ...
    this.boardFrame.fillStyle(0x040d15, 0.99);
    this.boardFrame.fillRoundedRect(
      boardX - outer,
      boardY - outer,
      boardSize + outer * 2,
    ...
    this.boardFrame.lineStyle(3, 0x1b9da8, 0.75);
    ...
    this.boardFrame.lineStyle(1, 0x80fff0, 0.24);
    this.boardFrame.strokeRoundedRect(
      boardX - outer + 7,
      boardY - outer + 7,
      boardSize + outer * 2 - 14,
      boardSize + outer * 2 - 14,
      radius - 6
    );

    // Inner luminous playing chamber.
    this.boardFrame.fillStyle(0x071923, 1);
    this.boardFrame.fillRoundedRect(
      boardX - 2,
      boardY - 2,
      boardSize + 4,
      boardSize + 4,
      Math.max(12, cell * 0.15)
    );

    // Cell grid.
    this.boardFrame.lineStyle(1, 0x63ddd4, 0.075);
    ...
    // Subtle diagonal maze lanes.
    this.boardFrame.lineStyle(1, 0x7c82ff, 0.055);
    for (let i = 0; i < COLS; i += 2) {
      this.boardFrame.lineBetween(
        boardX + i * cell,
        boardY,
        boardX + boardSize,
        boardY + (i + 1) * cell
    ...
    this.boardFrame.lineStyle(2.5, 0x74ffe9, 0.85);
    const b = Math.max(14, cell * 0.24);
    const left = boardX - outer + 10;
    const right = boardX + boardSize + outer - 10;
    const top = boardY - outer + 10;
    const bottom = boardY + boardSize + outer - 10;
  ...
  private drawCandy(
    ...
    aura.fillCircle(0, 0, r * (special ? 1.42 : 1.24));
    ...
    if (special) {
      ...
      specialRing.strokeCircle(0, 0, r * 1.17);
  ...
  private showIntroScreen() {
    ...
    const overlay = this.add.rectangle(0, 0, width, height, 0x010409, 0.93)
      .setOrigin(0)
      .setDepth(300);

    const panelW = Math.min(width - 26, 470);
    const panelH = portrait ? Math.min(height - 28, 620) : Math.min(height - 28, 450);
    const panelX = width / 2 - panelW / 2;
    const panelY = height / 2 - panelH / 2;

    const panel = this.add.graphics().setDepth(301);
    panel.fillStyle(0x06131d, 0.99);
    panel.fillRoundedRect(panelX, panelY, panelW, panelH, 30);
    panel.lineStyle(2, 0x55ead9, 0.72);
    ...
    // Cinematic top ornament.
    panel.fillStyle(0x5ee8d7, 0.72);
    panel.fillRoundedRect(width / 2 - 70, panelY + 18, 140, 3, 2);

    const sector = this.add.text(width / 2, panelY + 45, "SECTOR 01 // INITIALIZATION", {
      fontFamily: "Trebuchet MS, Arial, sans-serif",
      fontSize: `${Math.min(10, width * 0.024)}px`,
      fontStyle: "bold",
      color: "#6fe7d9",
      letterSpacing: 3,
    }).setOrigin(0.5).setDepth(302);

    const title = this.add.text(width / 2, panelY + 82, "MAZE HUNTER", {
      fontFamily: "Trebuchet MS, Arial, sans-serif",
      fontSize: `${Math.min(46, width * 0.095)}px`,
      fontStyle: "bold",
      color: "#f5ffff",
      stroke: "#02080e",
      strokeThickness: 7,
    ...
    const subtitle = this.add.text(width / 2, panelY + 119, "CRYSTAL MAZE", {
      fontFamily: "Trebuchet MS, Arial, sans-serif",
    ...
    const introLine = this.add.text(width / 2, panelY + 151, "HUNT • MATCH • CHAIN • ESCAPE", {
      fontFamily: "Trebuchet MS, Arial, sans-serif",
      fontSize: `${Math.min(10, width * 0.024)}px`,
      fontStyle: "bold",
      color: "#8ca5b1",
    ...
    const steps = [
      ["01", "SWIPE / TAP", "Move neighboring crystals to create a line of three."],
      ["02", "CREATE POWER", "Four, five, and T/L matches create special crystals."],
      ["03", "CLEAR THE MISSION", "Build your score before moves or the timer run out."],
    ];

    const stepObjects: Phaser.GameObjects.GameObject[] = [];
    const stepTexts: Phaser.GameObjects.Text[] = [];
    const startY = panelY + (portrait ? 205 : 190);
    const stepGap = portrait ? 93 : 68;

    steps.forEach(([num, heading, body], index) => {
      const y = startY + index * stepGap;

      const card = this.add.graphics().setDepth(302);
      card.fillStyle(0x0b202b, 0.9);
      card.fillRoundedRect(panelX + 20, y - 31, panelW - 40, 70, 16);
      card.lineStyle(1, 0x3acfc3, 0.18);
      card.strokeRoundedRect(panelX + 20, y - 31, panelW - 40, 70, 16);

      const circle = this.add.circle(panelX + 50, y + 2, 18, 0x122f3b, 1)
        .setStrokeStyle(1.5, 0x65eadc, 0.7)
        .setDepth(303);

      const n = this.add.text(panelX + 50, y + 2, num, {
        fontFamily: "Trebuchet MS, Arial, sans-serif",
        fontSize: "11px",
        fontStyle: "bold",
        color: "#ffffff",
      ...
      const h = this.add.text(panelX + 78, y - 12, heading, {
        fontFamily: "Trebuchet MS, Arial, sans-serif",
        fontSize: `${Math.min(13, width * 0.031)}px`,
        fontStyle: "bold",
        color: "#f3ffff",
        letterSpacing: 1,
      }).setOrigin(0, 0.5).setDepth(303);

      const b = this.add.text(panelX + 78, y + 14, body, {
        fontFamily: "Trebuchet MS, Arial, sans-serif",
        fontSize: `${Math.min(11, width * 0.026)}px`,
        color: "#8faab5",
        wordWrap: { width: panelW - 112 },
        lineSpacing: 3,
      }).setOrigin(0, 0.5).setDepth(303);

      stepObjects.push(card, circle, n, h, b);
      stepTexts.push(h, b);
    });

    const buttonY = panelY + panelH - 52;
    const button = this.add.rectangle(width / 2, buttonY, Math.min(panelW - 42, 330), 58, 0x18a79f, 1)
      .setInteractive({ useHandCursor: true })
      .setDepth(303);

    button.setStrokeStyle(2, 0x8cfff4, 0.75);

    const buttonText = this.add.text(width / 2, buttonY, "ENTER THE CRYSTAL MAZE", {
      fontFamily: "Trebuchet MS, Arial, sans-serif",
      fontSize: `${Math.min(16, width * 0.038)}px`,
      fontStyle: "bold",
      color: "#ffffff",
    ...
    const introObjects: Phaser.GameObjects.GameObject[] = [
      overlay, panel, sector, title, subtitle, introLine,
      ...stepObjects, button, buttonText,
    ];

    introObjects.forEach((obj) => ((obj as unknown) as Phaser.GameObjects.Components.Alpha).setAlpha(0));

    gsap.to(introObjects, {
      alpha: 1,
      duration: 0.7,
      stagger: 0.035,
      ease: "power2.out",
    ...
    gsap.fromTo(
      title,
      { y: panelY + 104, scale: 0.82 },
      { y: panelY + 82, scale: 1, duration: 0.9, ease: "back.out(1.7)" }
    );
    ...
    button.on("pointerup", () => {
      ...
      gsap.to(introObjects, {
        ...
        ease: "power2.in",
        onComplete: () => {
          introObjects.forEach((obj) => obj.destroy());
        },
      });
      ...
      const pulse = this.add.text(width / 2, height * 0.36, "HUNT STARTED", {
        fontFamily: "Trebuchet MS, Arial, sans-serif",
        fontSize: `${Math.min(27, width * 0.058)}px`,
        fontStyle: "bold",
        color: "#72ffe3",
        stroke: "#06131c",
        strokeThickness: 6,
  ...
  private showEndScreen(won: boolean) {
    ...
    const detail = this.add.text(
      width / 2,
      panelY + 192,
      `LEVEL ${this.level.toString().padStart(2, "0")}   •   BEST COMBO ×${this.bestCombo}`,
      {
        fontFamily: "Trebuchet MS, Arial, sans-serif",
